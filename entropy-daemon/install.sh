...
echo ""
echo "Next steps:"
echo "  1. Copy config: cp config.toml config-mainnet.toml"
echo "  2. Edit config-mainnet.toml — set keypair_path and node settings"
echo "  3. Register:   cd ../entropy-contract && node register_node.js 'My Node'"
echo "  4. Test:       CONFIG_PATH=./config-mainnet.toml SUBMIT_SCRIPT=./submit_entropy_mainnet.js python3 daemon.py"
echo "  5. Enable:     systemctl --user enable geiger-entropy"
echo "  6. Start:      systemctl --user start geiger-entropy"
echo "  7. Check API:  curl http://localhost:8746/health"
echo ""
echo "⚠️  Keep minimum 25 XNT in your operator wallet for slash protection!"
